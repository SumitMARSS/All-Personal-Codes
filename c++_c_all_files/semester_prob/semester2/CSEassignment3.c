...
//user define function to insert data at any place
struct node *insbagnode(struct node *point, int val, int add)
{
...
struct node *count(struct node *point)
{
    ...
    printf("Total number in the given list: %d", sum);
}
...
//user define function to delete data from any pos you want
struct node *delagn(struct node *point, int val)
{
    struct node *avail;
    if ( start == NULL)
        printf("UNDER FLOW");
    point = start;
    avail = point;
    while(point->inform != val)
    {
        avail = point;
        point = point->link;
    }
    avail->link = point->link;
    point->link = avail;
    avail = point;
    printf("Data is deleated sucessfully\n");
}
int main()
{
    struct node *point;
    ...
    while (1)
    {
        ...
        printf("Enter 4 to insert data at specified position.\n");
        printf("Enter 5 to counting the number of data you entered.\n");
        printf("Enter 6 to delete data from beginning.\n");
        printf("Enter 7 to delete data from the end.\n");
        printf("Enter 8 to delete data from specified position.\n");
        printf("Enter 9 tO EXIT/END.\n");
        ...
        switch (choice)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                printf("Enter the place at node you want to add another data after that node:");
                scanf("%d", &value);//taking input the place at which 
                printf("Enter the value to add in the data list :");
                scanf("%d", &add);//taking value to enter in list at given place
                insbagnode(point, value, add);//using user defined fun
                break;
            case 5:
            ...
            case 7:
                ...
                break;
            case 8:
                printf("Enter the position to delete from the data list :");
                scanf("%d", &value);
                delagn(point, value);//using user defined fun
                break;
            case 9:
